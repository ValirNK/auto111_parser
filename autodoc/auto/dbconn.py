...
class DB_api:
    ...
    def get_value(self, id):
        if self.cursor != False:
            with self.cursor as cursor:
                sql = f"SELECT * FROM needs_8 WHERE id = {id}"
                cursor.execute(sql)
                value = cursor.fetchone()
                return value

    def set_status(self, sku):
        if self.cursor != False:
            with self.cursor as cursor:
                sql = f"UPDATE needs_8 SET status = 0 WHERE part_sought = '{sku}'"
                cursor.execute(sql)
...
#         # print(i)
#         b = Needs_8(
#             id_1c_part = i['id_1c_part'],
#             id_1c_doc = i['id_1c_doc'],
#             part_sought = i['part_sought'],
#             brand_sought = i['brand_sought'],
#             status = i['status']
#         )
#         b.save()
